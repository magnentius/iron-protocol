...
| **IRCM / Chaff** | **IRCM suite**: 2 EP per activation, no magazine — a powered jammer, so it never runs dry but it spends the same banked charge an Overcharge needs. **Chaff**: free, Ammo Die 1d6, roughly six releases. Each negates one attack on its band (IR / Radar) on a Check. Smoke costs 1 EP because you deploy it on your own turn. |
| **Ammo Die** | Everything runs **Empty permanently**, never reloaded. Autocannon: Empty on **1** (~6 bursts) or **1-3** on Full Auto (~2 barrages). Missiles, Chaff, Smoke, Jump propellant: Empty on **1-2** (~3 uses). A Frame with no volatile stores left can no longer cook off. |
...
*A weapon cannot fire at all without a lock on its band. Terrain (woods, buildings, elevation) denies a band outright; every deployed countermeasure — IRCM, Chaff, Smoke, ECM, Adaptive Skin — contests it on a **Countermeasure Check (4+)**.*

| Weapon | Needs | Silenced by |
| :--- | :---: | :--- |
| **Laser** | VIS | Woods outright; Smoke or Visual-mode Skin on a Check |
| **Thermal Lance** | IR | Target running cold outright; IRCM suite or Infrared-mode Skin on a Check |
...
| **IRCM Suite** | *Clear* | **Negates the attack** | *Clear* |
...
| **Honesty** (Makoto) | No Skin, Smoke, IRCM, Chaff or ECM — yours or an ally's | Enemy **Adaptive Skins and ECM do not function** against your locks (no Check rolled). Chaff/IRCM/Smoke still contest normally |
