...
* 🧪 **[playtest.md](playtest.md)**: Eight scenarios used as a test suite for the rules engine, from a 1v1 introductory duel up to a 4v4 slugfest, each written to stress a specific system — armor degradation, sensor stealth, the Overcharge economy, the Vow system, and action economy.
* 📱 **[Battle Tracker](docs/)**: A mobile web app that carries the bookkeeping for you — energy and capacitor charge, locational Armor DR, Ammo Dice, and the permanent critical-hit effects that are easy to lose track of, with a guided attack resolver and live sync between two phones via a room code. See **[docs/README.md](docs/README.md)** to run or deploy it.
  > ⚠️ **Out of date.** The tracker still implements the pre-overhaul damage model (Evasion, Internal Structure, subtractive armor, counted ammunition) and **cannot run the current rules**. Play from the frame sheets until it is rebuilt.
