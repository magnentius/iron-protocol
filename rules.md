...
- **Movement Limit**: The maximum number of hexes a Frame can enter (via walking, reversing, or jumping) in a single turn. These are the *maximum* values permitted for each weight class (see the Chassis Limits Table in Section 7.2); an individual Frame may be rated lower:
  - **Light**: up to 7 hexes (approx. 75 km/h)
  - **Medium**: up to 5 hexes (approx. 54 km/h)
  - **Heavy**: up to 4 hexes (approx. 43 km/h)
  - **Assault**: up to 3 hexes (approx. 32 km/h)
...
- **Internal Structure (IS)**: The structural framework points for each of the 6 hit locations. If a location's IS is reduced to 0, that location is destroyed. *(Each leg is tracked separately: a profile listing "Legs: 8 IS" means 8 IS in the Left Leg and 8 IS in the Right Leg.)*
...
- **Recommended Matchup — Vanguard Mirror**: Both players field an **IF-55M-1 "Vanguard"** (430 pts). A mirror match is the clearest way to learn the engine: with identical Frames, every difference in the outcome comes from pilot decisions rather than from the matchup. It teaches the three things that decide games — banking EP for Overcharges, trading movement for Evasion, and choosing between a Single Burst and Full Auto — without any asymmetry to untangle at the same time.
- **Second Game — Vanguard vs. Specter**: Once both players are comfortable, field the **IF-55M-1 "Vanguard"** (430 pts) against the **IF-45M-1 "Specter"** (435 pts). This is a deliberately asymmetric matchup and it rewards knowing the Specter's line:
...
- **Collisions & Blocking**: If a Frame's movement path would enter a hex occupied by another Frame, a collision occurs. The moving Frame immediately stops in the last unoccupied hex, its activation ends, and both frames suffer damage.
  - **Collision Damage**: Both the moving Frame and the stationary target Frame suffer damage to a random location determined by rolling on the Hit Location Table individually. Evasion (EVA) points do **not** reduce collision damage, as the impact is physical and unavoidable. **Armor DR is applied normally** to each hit, and a penetrating collision degrades that location's Armor DR by 1 and triggers a Critical Hit roll, exactly as a weapon hit would.
  - **Deliberate Ramming**: A pilot may intentionally drive into an occupied hex. Note that the damage dice pool is derived from the *moving* Frame's Mass Value, and **both Frames roll that same pool** — a light Frame that rams a heavy one will almost always come off worse.
...
| **Urban (Building)** | Impassable (Ground) | Solid Barrier | Structures block all locks (VIS/IR/Radar) up to height. Heavy Cover (+2 EVA) if adjacent. Roofs (Level 2) can only be reached by **Jump Jets**, and count as their own hex at that elevation. |
...
- **Flares**: Limited charges (typically 3). Expending 1 charge completely negates a single incoming attack, but only when **all** of the following are true:
  - The attacker resolved the attack using an **Infrared (IR) lock**. If the attacker held a Visual or Microwave lock on the target as well, they simply use that instead and the Flare is wasted — declare Flares *after* the attacker states which spectrum they are firing on.
  - The attack came from the target's **Front or Rear Hit Zone**. Attacks from a Side Hit Zone arrive too far off the flare's decoy axis and **cannot be deflected**.
  - Flares are launched **before** the Hit Location roll.

  Flares are consequently at their best against Infrared-Guided missiles, which have no fallback spectrum.
...
- **Area of Effect (AoE)**: Damage designated as **AoE** (such as Missile Cluster and Splash damage) blankets a wide area and completely bypasses Evasion. Do not subtract the target's EVA points from AoE damage rolls. *Because terrain Cover is expressed as bonus EVA, Cover is bypassed as well — there is no hiding behind a tree from a cluster warhead. Armor DR still applies normally.*
...
| **Rail Gun** | Heavy | 0 (+6) | 5 Slugs (Inert) | 1 Turn | 3d6 + 10 | Any | AP 3, Overcharge | Inert slugs. **Requires Overcharge to fire**: [+6 EP, which must come from the Capacitor]. Because firing always involves an Overcharge, the Rail Gun always enters a 1-Turn Cooldown. |
...
The Disruptor Cannon deals **no damage**. It fires a directed-energy pulse that bypasses both Evasion and Armor DR entirely, and its effect depends on the location rolled on the Hit Location Table:
...
#### Rapid Fire, Evasion and Armor Degradation
When a Rapid Fire weapon is fired:
- **Evasion negates whole hits, highest first.** Each point of the target's Evasion cancels the single highest damage die remaining in the pool. Cancelled dice are discarded before armor is applied.
- **Every surviving die is resolved against the Armor DR the location had when the attack was declared.** Do not re-apply degradation between dice of the same attack.
...
- **Roll one Critical Hit for the attack** if the location's Internal Structure took 1 or more points of damage in total, not one per die.
...
- **Tracer Rounds (Target Marking)**: Apply a **-1 damage penalty** to every 1d6 damage die rolled for the weapon (minimum 0 damage per die). However, if even a single tracer round hits the target, the target becomes "Painted". All subsequent attacks from *any friendly Frame (including the attacker)* ignore 1 point of the target's Evasion (EVA) for the rest of the Combat Phase. *A tracer die "hits" if it was not negated by Evasion — it paints the target even if the round is then stopped by Armor DR and deals no damage. Marking the target is the point of the round; the damage is incidental.*
...
- **6+: Leg Severed**. The leg is destroyed outright, regardless of how much Internal Structure remained. The Frame falls Prone immediately and is permanently crippled (see Section 6.5.4). It may fight on, and may attempt to haul itself upright, but it will never walk again.
...
If a Leg's Internal Structure is reduced to 0 — or a **Leg Severed** critical is rolled — the leg is blown off.
...
- **The Constraint**: Hit locations are rolled, not chosen, so this vow governs what the pilot does with the roll. While the target Frame still has **any** Arm or Leg with remaining Internal Structure, a Mercy pilot who rolls a **Head or Torso** hit against it must **pull the shot**: the attack deals no damage and no Critical Hit is rolled, though the EP and ammunition are still spent. Once every limb on the target has been reduced to 0 Internal Structure, the pilot may strike the core freely.
...
- **Internal Structure**: Head: 4 | Torso: 8 | Left Arm: 4 | Right Arm: 4 | Each Leg: 5
- **Equipped Systems**:
...
- **Internal Structure**: Head: 5 | Torso: 10 | Left Arm: 6 | Right Arm: 6 | Each Leg: 8
- **Equipped Systems**:
  - Standard Sensor Suite (Visual [VIS]/Infrared [IR]/Microwave [Radar])
...
- **Internal Structure**: Head: 6 | Torso: 12 | Left Arm: 8 | Right Arm: 8 | Each Leg: 10
- **Equipped Systems**:
  - Standard Sensor Suite (Visual [VIS]/Infrared [IR]/Microwave [Radar])
  - ECM Suite [Medium HP]
...
- **Internal Structure**: Head: 7 | Torso: 16 | Left Arm: 10 | Right Arm: 10 | Each Leg: 12
- **Equipped Systems**:
  - Standard Sensor Suite (Visual [VIS]/Infrared [IR]/Microwave [Radar])
  - Flare Launcher (3 charges) [Light HP]
...
- **Internal Structure**: Head: 8 | Torso: 20 | Left Arm: 12 | Right Arm: 12 | Each Leg: 15
- **Equipped Systems**:
  - Standard Sensor Suite (Visual [VIS]/Infrared [IR]/Microwave [Radar])
  - Smoke Launcher (2 charges) [Light HP]
  - Flare Launcher (3 charges) [Light HP]
