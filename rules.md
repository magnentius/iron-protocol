...
- **Second Game — Vanguard vs. Specter**: Once both players are comfortable, field the **IF-55M-1 "Vanguard"** (455 pts) against the **IF-45M-1 "Specter"** (435 pts). This is a deliberately asymmetric matchup and it rewards knowing the Specter's line:
  ...
  - **The whole plan runs on the Microwave band.** The Disruptor needs a Radar lock, and the Vanguard carries both a Chaff Dispenser and an ECM suite. A Chaff cartridge negates a shot outright, but there are only about three of them; an ECM suite runs all game for 2 EP a turn but only contests each shot on a **Jamming Check**, so roughly half the Specter's pulses still land. The Specter's opening problem is therefore electronic, not structural: bait out the Chaff early, then accept that every Disruptor shot into the ECM umbrella is a coin flip.
  - The Specter's **Adaptive Skin decides which of the Vanguard's guns still works.** Suppressing Visual forces every Laser shot to survive a Jamming Check, so about half of them simply fail to acquire; the Vanguard leans on its Autocannon and must close to make it count. Cloaking a second band costs an Overcharge and leaves the Skin offline entirely next round, so it is a spike, not a posture.
...
- **Attack Sequence**:
  ...
  4. **Defender's Countermeasures**: Before any location is rolled, the defender resolves any countermeasure it wishes to use.
     - **Flares / Chaff**: a launched cartridge negates the attack **outright** if the attack used the matching lock (IR for Flares, Radar for Chaff). No roll is made. Afterwards, roll the launcher's Ammo Die.
     - **Jamming Check**: if the target is covered by an active **ECM** suite, or by an **Adaptive Skin** tuned to the band this attack was made on, the defender rolls **1d6**. On a **4, 5 or 6** the lock fails and the attack is negated entirely; on a **1, 2 or 3** the attacker burns through and the attack continues. The suite is not expended either way (see Section 4.2).
...
1. **Visual (VIS)**: Shortest wavelength (approx. 380–700 nm). Housed in high-resolution optical cameras.
   ...
   - *Blocked by*: Smoke templates or heavy forests outright; an Adaptive Skin in Visual mode contests it on a **Jamming Check**.
2. **Infrared (IR)**: Medium wavelength (approx. 700 nm–1 mm). Thermal sensors detecting heat signatures.
   ...
   - *Blocked by*: Flares outright; an Adaptive Skin in Infrared mode contests it on a **Jamming Check**.
3. **Microwave (Radar)**: Longest wavelength (approx. 1 mm–1 m). Active microwave radio detection.
   ...
   - *Blocked by*: Chaff or solid Elevation (Hills/Mountains) outright; an active ECM suite or an Adaptive Skin in Microwave mode contests it on a **Jamming Check**.

### 4.2 Stealth & Defensive Countermeasures
Frames can run active systems to deny locks and hide from sensors:

- **The Jamming Check (sustained suites only)**: An always-on suite degrades a lock; it does not guarantee a miss. When an attack is declared against a Frame covered by an active **ECM** suite or an **Adaptive Skin** tuned to that attack's band, the defender rolls **1d6 before the Hit Location roll**. On a **4, 5 or 6** the lock fails and the attack is negated entirely. On a **1, 2 or 3** the attacker burns through the jamming and the attack proceeds as normal. A failed check does not switch the suite off — it remains active and is rolled again against the next attack.

  *Flares, Chaff and Smoke never roll a Jamming Check.* They are finite stores, and running out is what balances them; a cartridge fired at the right moment simply works. A sustained suite has no such limit, so it is governed by probability instead. One system is rationed, the other is unreliable.

- **Electronic Countermeasures (ECM)**: Costs **2 EP** to activate in the Energy Phase. Contests Microwave (Radar) detection and locks on the host Frame (0-hex radius) — each attack tested against it requires a **Jamming Check**. **Overcharge** [+1 EP per +1 hex radius]. All friendly Frames within the active radius are protected. *(Note: Overcharging triggers the mandatory 1-Turn Cooldown on the ECM suite).*
- **Flares**: (Ammo Die: 1d6, **no EP cost**). Activating the launcher completely negates a single incoming attack, but only when **all** of the following are true:
  ...
  After launching Flares, roll the Ammo Die (1d6). On a **1 or 2**, the launcher is empty for the rest of the battle.
- **Chaff Dispensers**: (Ammo Die: 1d6, **no EP cost**). Activating the dispenser completely negates a single incoming attack, but only when **all** of the following are true:
  ...
  After launching Chaff, roll the Ammo Die (1d6). On a **1 or 2**, the dispenser is empty for the rest of the battle.
- **Smoke Launchers**: (Ammo Die: 1d6, **1 EP**). During the Activation Phase, a Frame may spend 1 EP to deploy a Smoke cloud in its current hex, or any of the 6 surrounding adjacent hexes (place a Smoke token). The smoke template blocks Visual (VIS) LOS and Visual locks through that hex for as long as it remains on the board. Infrared (IR) and Microwave (Radar) sensors are unaffected and can scan through smoke unimpeded. After launching Smoke, roll the Ammo Die (1d6). On a **1 or 2**, the launcher is empty for the rest of the battle.

> *Why Smoke costs energy and Flares and Chaff do not: Smoke is **proactive**. You deploy it on your own activation, in a hex of your choosing, and you can budget the point of energy for it. Flares and Chaff are **reactive** — fired during an opponent's attack, at a moment you cannot predict, against a lock you did not choose. Charging energy for a reaction would mean the question "can I defend myself?" was settled before you knew who was shooting or with what. The Ammo Die is the whole cost: roughly three charges, and once they are gone they are gone. Decide whether **this** attack is worth one of them.*
- **Adaptive Skin**: Costs **2 EP** to activate in the Energy Phase. The Frame tunes its coating to absorb **one** sensor spectrum (Microwave, IR, or VIS), contesting detection and locks on that spectrum — each attack made on the cloaked band requires a **Jamming Check**. **Overcharge**: [+2 EP to cloak a 2nd spectrum simultaneously]. *(Note: Overcharging triggers the mandatory 1-Turn Cooldown, meaning the Adaptive Skin will be completely offline during the following round, leaving the Frame fully exposed on all spectrums).*
...
| **Flares / Chaff / Smoke** | 1 or 2 | 3 uses |
...
*The Disruptor produces fewer criticals than a comparable gun, and it can never breach a location by itself. What it offers is certainty **once it has its lock**: against a fast Frame in heavy cover behind thick plate — a target every other weapon struggles to touch — neither armor nor speed nor terrain cover reduces the effect by one point. That certainty is bought on the Microwave band, and it is the band most easily denied: a Chaff cartridge takes the Disruptor off the board for that attack outright, and an ECM suite contests every shot on a Jamming Check. It is a weapon that either works completely or not at all.*
