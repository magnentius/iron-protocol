...
- **Warheads**:
  ...
  - *EMP (Electromagnetic Pulse)*: Detonates in a massive sphere. Targets a **specific hex** rather than a single Frame. **All Frames** within the target hex and the 6 surrounding adjacent hexes suffer the effect, **including your own**. Deals no physical damage, rolls no damage dice, and **ignores Armor DR entirely** — plate is no protection against it, and stripped plate earns the attacker no bonus.
    ...
    - **The target hex — a Sensor Critical.** Every Frame in the hex itself rolls **1d6**: on a **1–2** *Sensor Ghosting*, on a **3–4** *Sensor Calibration Drift*, on a **5–6** *Sensor Array Destroyed*. These are the three sensor results of the Head Critical Table and resolve exactly as they do there, including the further 1d6 for which array burns out. **Do not mark the Head table** — a pulse breaches nothing, and a marked slot would let repeated pulses cascade toward a Structural Fracture or a Pilot K.O.
    - **The six surrounding hexes — one band suppressed.** Roll **1d6** for each Frame: on a **1–2** the Thermal (IR) array, on a **3–4** the Optical (VIS) array, on a **5–6** the Microwave (Radar) array. That Frame cannot lock on the affected band, and so cannot fire any weapon needing it, **until the End Phase**. Nothing is destroyed; it comes back.
    - **Every Frame caught, in either zone, has its Tactical Datalink jammed** until the End Phase.

  > *What the warhead is for. It rolls no damage and can never destroy anything, so it does not win a fight — it decides what kind of fight the next few turns will be. Because it ignores armour completely it is at its best **first**, before a shot has been fired: a lance that opens the game blind on a spectrum, disconnected from its datalink, and with one of its number carrying a permanent sensor fault, is fighting a different battle from the one it planned. Placement is the whole skill. The hex you choose takes lasting harm; the ring around it only stumbles for a turn — and your own Frames standing in either are no more exempt than theirs.*
